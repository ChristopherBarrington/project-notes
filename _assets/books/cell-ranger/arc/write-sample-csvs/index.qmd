...
Cell Ranger ARC [@cellranger; @cellranger_atac] needs to distinguish between the assay types provided for each sample. The user-provided table `inputs/library-metadata.csv` defines which libraries provide either GEX or ATAC assays for a sample.
...
#| eval: false
#| file: bin/write-sample-csvs.R
#| filename: bin/write-sample-csvs.R
```
...
Rscript bin/write-sample-csvs.R \
--fastq-path etc/fastq \
--library-metadata-csv inputs/library-metadata.csv \
--outputs-path outputs/sample-csvs
```
