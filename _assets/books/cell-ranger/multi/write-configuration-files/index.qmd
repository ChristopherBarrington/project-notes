...
1. (optional) Cell Ranger [probe set CSV] file `inputs/probe-set.csv`. If missing, the output probe set created previously can be used, and can be linked automatically.
...
from_file <- 'inputs/probe-set.csv'
to_file   <- 'etc/probe-set.csv'

if(! file.exists(from_file)) {
	if(is_link(to_file))
		link_delete(to_file)

	file.path('../..', 'create-probe-set', 'outputs/probe-set.csv') -> from_file

	c('::: {.callout-warning}',
	  str_glue('The probe set was not provided, so `{from_file}` was linked.'),
	  ':::') |> cat(sep='\n')
}
...
#| eval: false
#| file: bin/write-library-configuration-files.sh
#| filename: bin/write-library-configuration-files.sh
```

```{bash run-write-library-configuration-files-script}
#| code-summary: Run the `bash` script to write library configuration files
#| eval: true
#| results: hide

FASTQ=etc/fastq \
OUTPUTS=outputs/configs \
PROBE_SET=etc/probe-set.csv \
REFERENCE=inputs/reference \
SAMPLE_SHEETS=inputs/sample-sheets \
bash bin/write-library-configuration-files.sh
```
