...
```{bash setup_wd, cache=FALSE, echo=FALSE, code=readLines(system.file('chunks/working_directory.sh', package='project.notes.tools'))}
```
...
```{r show_list_references, cache=FALSE, child=system.file('chunks/list_references.Rmd', package='project.notes.tools')}
```
