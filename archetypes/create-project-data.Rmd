...
Here, a connection to the Ensembl archive is created and saved to the `data-repository`.
...
#! can optionally be a named vector
list() -> factor_levels
...
complete_sample_sheet[names(factor_levels)] %<>%
  Map(data=., levels=factor_levels, function(data, levels)
    data %>%
      fct_relevel(levels) %>%
      when(names(levels) %>% is.null() ~ .,
           TRUE ~ fct_recode(., !!!levels)))
