...
c(deseq2='10.1186/s13059-014-0550-8',
  ...
  upsetr='10.1093/bioinformatics/btx364') -> bibliography_doi
```
...
[project data post sample sheet]: {{< ref "create-project-data#read-and-parse-metadata" >}}
...
The sample sheet was defined in [{{< post-title "create-project-data" >}}][] and is shown below.

```{r text_n_rows_project_sample_sheet, echo=FALSE, results='asis'}
project_sample_sheet %>%
  nrow() %>%
...
validated_genes %<>%
  when(class(.)=='character'~getBM(attributes=c('external_gene_name', 'ensembl_gene_id'), filters='external_gene_name', mart=mart),
       TRUE~.) %>%
  when(nrow(.)==0~c(),
       TRUE~deframe(.))
```
...
```{r get_de_gene_sets, dependson='collect_results'}
all_dds %>%
...
```{r show_venn_compare_de_gene_sets_plots, dependson='get_de_gene_sets', fig.width=3, fig.height=3}
palette <- brewer.pal(name='Dark2', n=3)
category_distances <- c(0.1, 0.1, 0.05)

venn.diagram(x=de_gene_sets,
             filename=NULL,
             cat.fontface='bold',
             cat.col=palette,
...
```{r show_upsetr_compare_de_gene_sets_plots, dependson='get_de_gene_sets', echo=FALSE, fig.width=4, fig.height=3}
fromList(de_gene_sets) %>%
  upset()
```

A table of [gene responses in each comparison](files/de_gene_sets_comparison.xlsx) is created and a snippet shown below.

```{r show_de_gene_sets, dependon='get_de_gene_sets'}
de_gene_sets %>%
...
```{r set_ma_plot_labels, dependson='set_validated_genes'}
ma_plot_labels <- validated_genes
```

```{r get_ma_plots, dependson=c('collect_results', 'set_ma_plot_labels')}
make_ma_plot <- function(results, lfc='shrunk.log2FoldChange', maxLFC=3, labels)
  results %>%
    ...
    arrange(desc(padj)) %>% {
      ...
      geom_text_repel(data=filter(., !is.na(label)), mapping=aes(label=label), box.padding=0.5, max.overlaps=Inf, colour='black') +
...
for(i in names(plots_list)) {
  sprintf('{{< tab "%s" >}}', i) %>% cat()
  plots_list[[i]] %>% resize_and_show(width=2.5, clip=FALSE)
  cat('{{< /tab >}}')
}
```
{{< /tabs >}}
