...
c(deseq2='10.18129/B9.bioc.DESeq2',
  tximport='10.18129/B9.bioc.tximport') -> bibliography_doi
```
...
[project data post]: {{< ref "create-project-data" >}}
[nfrnaseq post]: {{< ref "nfcore-rnaseq" >}}

<!-- end of initial set up chunks -->

Gene expression has been quantified by the [nf-core/rnaseq][nfcore rnaseq] pipeline in the [{{< post-title "nfcore-rnaseq" >}}][nfranseq post] post and will now be read into a [DESeq2][deseq2 doi] `dds` object for downstream analysis. The object will be saved and can be loaded in future posts. We can then examine relationships between datasets and test for differential expression between samples.
...
file.path(project_path, 'analysis', 'nfcore-rnaseq', 'results', 'star_rsem') %>%
  ...
  set_names() %>%
  set_names(basename) %>%
  set_names(str_remove, '.genes.results') %>%
...
getBM(mart=mart, attributes=c('ensembl_gene_id', 'external_gene_name', 'description', additional_attributes)) %>%
...
The `project_sample_sheet` was created in the [{{< post-title "create-project-data" >}}][project data post] post and contains a description of each sample using metadata from the ASF-supplied sample sheet, the experimental table and any other provided information. It is loaded into this post using the `datarepository` package. Row names of `project_sample_sheet` should be the names of the datasets for use as a `colData` object.

Now a DESeq2 (v`r packageVersion('DESeq2')`) dataset object is created from the `tximport` list using [`DESeq2::DESeqDataSetFromTximport`][deseq2 vignette tximport] and the `project_sample_sheet`.

A dummy design is applied here, it will be replaced later when the sample comparisons are defined.

```{r make_dds, dependson=c('get_ensembl_to_name', 'make_txi')}
...
```{r distance_matrix, dependson='make_dds'}
...
{{< tab "All datasets" >}}
```{r pca_plots_all_datasets_scatter, dependson=c('make_dds', 'pca_plot_functions'), echo=FALSE}
...
{{< tab "SUBSET datasets" >}}
```{r pca_plots_SUBSET_datasets_scatter, dependson=c('make_dds', 'pca_plot_functions'), echo=FALSE}
...
```{r collect_results, dependson=c('make_dds', 'get_comparisons_config', 'function_to_run_deseq2')}
...
```{r save_results, dependson='collect_results', echo=FALSE}
...
```{r set_validated_genes, echo=FALSE}
c() -> validated_genes
```
...
```{r get_feature_count_plots, dependson=c('set_validated_genes','collect_results'), eval=length(validated_genes)>0}
data(mart)

validated_genes %<>%
  getBM(attributes=c('external_gene_name', 'ensembl_gene_id'), filters='external_gene_name', mart=mart) %>%
  deframe()

plot_feature_counts <- function(dds, features, x) {
  if(missing(x))
    x <- metadata(dds) %>% pluck('contrast', 'group')
  ...
  counts(object=dds, norm=TRUE)[features,] %>%
    ...
    left_join(y=project_sample_sheet, by='Sample.limsid') %>%
...
```{r show_feature_count_plots, dependson='get_feature_count_plots', echo=FALSE, results='asis', eval=length(validated_genes)>0}
cat('{{< tabs "show_feature_count_plots" >}}\n')
...
```{r show_no_validated_genes_notice, echo=FALSE, results='asis', eval=length(validated_genes)==0}
cat('_*No validated genes were supplied!*_\n')
```
...
```{r function_to_make_dispersion_plots}
...
{{< tabs "plot_dispersion_plots_for_each_comparison" >}}
```{r plot_dispersion_plots_for_each_comparison, dependson=c('collect_results', 'function_to_make_dispersion_plots'), echo=FALSE, results='asis'}
for(i in names(all_dds)) {
  sprintf('{{< tab "%s" >}}', i) %>% cat()
  make_dispersion_plot(dds=all_dds[[i]]) %>%
...
```{r get_cooks_distance_for_each_comparison, dependson='collect_results'}
...
{{< tabs "show_cooks_distance_for_each_comparison" >}}
```{r show_cooks_distance_for_each_comparison, dependson='get_cooks_distance_for_each_comparison', echo=FALSE, results='asis'}
for(i in names(plots_list)) {
  sprintf('{{< tab "%s" >}}', i) %>% cat()
  {plots_list[[i]]$density + plots_list[[i]]$boxplot + plot_layout(widths=unit(3,'in'), height=unit(3/1.6, 'in'))} %>% show()
  cat('{{< /tab >}}')
}
```
{{< /tabs >}}
...
```{r get_independent_filtering_plots, dependson='collect_results'}
...
{{< tabs "show_independent_filtering_plots" >}}
```{r show_independent_filtering_plots, dependson='get_independent_filtering_plots', echo=FALSE, results='asis'}
for(i in names(plots_list)) {
  sprintf('{{< tab "%s" >}}', i) %>% cat()
  plots_list[[i]] %>% resize_and_show(width=3.5, clip=FALSE)
  cat('{{< /tab >}}')
}
```
{{< /tabs >}}
...
These heatmaps select the top `n` features by their variability across the `dds`.

```{r get_variable_features_heatmaps, dependson='collect_results'}
...
{{< tabs "show_variable_features_heatmaps" >}}
```{r show_variable_features_heatmaps, dependson='get_variable_features_heatmaps', echo=FALSE, results='asis'}
for(i in names(plots_list)) {
  sprintf('{{< tab "%s" >}}', i) %>% cat()
  plots_list[[i]] %>% resize_and_show(width=2, orientation='p')
  cat('{{< /tab >}}')
}
```
{{</ tabs >}}
...
{{< tabs "show_pca_plots_for_each_comparison" >}}
```{r show_pca_plots_for_each_comparison, dependson=c('collect_results', 'pca_plot_functions'), echo=FALSE, results='asis'}
comparisons_config %>%
...
```{r get_fdr_thresholds_plots, dependson=c('collect_results'), fig.height=30}
make_number_of_different_features_plot <- function(results_df)
  results_df %>%
    ...
    as.data.frame() %>%
    mutate(alpha=scientific(alpha),
           alpha=factor(alpha, levels={alpha %>% unique() %>% as.numeric() %>% sort() %>% scientific()}),
...
{{< tabs "show_fdr_thresholds_plots" >}}
```{r show_fdr_thresholds_plots, dependson='get_fdr_thresholds_plots', echo=FALSE, results='asis'}
for(i in names(plots_list)) {
  sprintf('{{< tab "%s" >}}', i) %>% cat()
  plots_list[[i]] %>% resize_and_show(size=1.6)
  cat('{{< /tab >}}')
}
```
{{</ tabs >}}

```{r show_fdr_thresholds_table, dependson=c('collect_results'), echo=FALSE}
results_summary %>%
...
```{r show_table_of_outcomes, dependson='collect_results', echo=FALSE}
all_dds %>%
...
```{r get_ma_plots, dependson='collect_results'}
make_ma_plot <- function(results, lfc='shrunk.log2FoldChange', maxLFC=3)
  results %>%
    as.data.frame() %>%
    ...
    arrange(desc(padj)) %>% {
      ggplot(data=.) +
      aes(x=baseMean, y=.lfc, colour=padj<=alpha, shape=abs(.lfc)>maxLFC) +
...
all_dds %>%
  llply(metadata) %>%
  llply(pluck, 'results') %>%
  llply(make_ma_plot, lfc='shrunk.log2FoldChange', maxLFC=3) -> plots_list
```

{{< tabs "show_ma_plots" >}}
```{r show_ma_plots, dependson='get_ma_plots', echo=FALSE, results='asis'}
for(i in names(plots_list)) {
  sprintf('{{< tab "%s" >}}', i) %>% cat()
  plots_list[[i]] %>% resize_and_show(width=3)
  cat('{{< /tab >}}')
}
```
{{< /tabs >}}
...
```{r get_de_features_heatmaps, dependson='collect_results'}
...
all_dds %>%
  ...
  llply(.parallel=TRUE, make_de_features_heatmap, annotations=c('treatment', 'donor_id')) -> plots_list
```

Heatmaps are plotted here for comparisons with at least `r min_de_features` significant feature(s).

{{< tabs "show_de_features_heatmaps" >}}
```{r show_de_features_heatmaps, dependson='get_de_features_heatmaps', echo=FALSE, results='asis'}
for(i in names(plots_list)) {
  sprintf('{{< tab "%s" >}}', i) %>% cat()
  plots_list[[i]] %>% resize_and_show(width=2, orientation='p')
  cat('{{< /tab >}}')
...
Here, the [DEGreport package][degreport bioc] is used to cluster expression profiles of the genes identified in the LRT test(s) as interesting.

```{r get_degPatterns, dependson='collect_results'}
...
```{r get_degPatterns_plots, dependson='get_degPatterns'}
...
{{< tabs "show_degPatterns_plots" >}}
```{r show_degPatterns_plots, dependson=c('get_degPatterns_plots'), echo=FALSE, results='asis'}
for(i in names(plots_list)) {
  sprintf('{{< tab "%s" >}}', i) %>% cat()
  plots_list[[i]] %>% resize_and_show(width=2, clip=FALSE)
  cat('{{< /tab >}}')
}
```
{{< /tabs >}}
...
* [significantly different features](files/dds_results.xlsx) (`dds_results`)
* [all features in the analysis](files/all_results.xlsx) (`all_results`) (this may be a very big file!)
...
```{r write_xlsx, dependson='collect_results', echo=FALSE}
all_dds %>%
  llply(.parallel=TRUE, function(dds) {
    colData(dds) %>%
      as.data.frame() %>%
      mutate(Sample.Name=str_c(col1, col2, sep='.')) %>%
    ...
    dds %>%
      metadata() %>%
      pluck('gene_information') -> gene_information

    results %>%
      left_join(y=gene_information) %>%
      left_join(y=norm_counts) %>%
      return()}) %T>%
  write.xlsx(file='files/all_results.xlsx') %>%
  lapply(filter, str_detect(outcome, '\\*')) %>%
  write.xlsx(file='files/dds_results.xlsx')
```
...
```{r write_degreport_xlsx, dependson='get_degPatterns'}
all_dds %>%
  llply(metadata) %>%
  llply(pluck, 'DEGreport') %>%
  discard(is.null) %>%
...
```{r write_metacore, dependson='collect_results', results='hide', echo=FALSE}
