...
::: {.content-visible when-meta="includes.conda"}
...
::: {.content-visible when-meta="includes.renv"}
...
[Download](files/renv.lock) the lockfile for the project to an empty directory. Start R version `r jsonlite::read_json('files/renv.lock')$R$Version`[^rversion] and use `renv::init()` to initialise a new renv-managed project.

[^rversion]: The major/minor version is important but a different patch version _should_ still work.
:::
